...
assert(/const failureKey = 'SIGNAL_DRIVE_FAIL_V2';/.test(utilSource), 'SIGNAL Drive scan has a Drive v2 failure cooldown key');
assert(/if \(cache\.get\(failureKey\)\) return \[\];/.test(utilSource), 'SIGNAL Drive scan honors failure cooldown');
assert(/cache\.put\(failureKey, '1', 300\)/.test(utilSource), 'SIGNAL Drive scan stores short failure cooldown');
...
assert(/const deletedPollerTriggerCount = _deleteProjectTriggersByHandler_\('runIntakePoller'\);/.test(utilSource), 'Daily automation trigger install clears the intake poller trigger family');
assert(/const scheduleHours = \[8, 11, 14, 16\];/.test(utilSource), 'Full app automation sync runs at 8am, 11am, 2pm, and 4pm');
assert(/ScriptApp\.newTrigger\('runIntakePoller'\)[\s\S]*\.everyMinutes\(INTAKE_POLLER_INTERVAL_MINUTES\)[\s\S]*\.create\(\);/.test(utilSource), 'Intake poller installs a recurring minute trigger');
assert(/function _isIntakePollerActiveWindow_\(now\)/.test(utilSource), 'Intake poller exposes an active-window guard');
assert(/hour >= INTAKE_POLLER_ACTIVE_START_HOUR && hour <= INTAKE_POLLER_ACTIVE_END_HOUR/.test(utilSource), 'Intake poller is bounded by configured business hours');
assert(/function _buildIntakeFolderFingerprint_\(\)/.test(utilSource), 'Intake poller builds a folder fingerprint before heavy work');
assert(/props\.getProperty\('INGESTION_IN_PROGRESS'\) === 'true'/.test(utilSource), 'Intake poller skips while ingestion is already running');
assert(/scan\.fingerprint === previousFingerprint/.test(utilSource), 'Intake poller no-ops when the incoming folder is unchanged');
assert(/processIncomingForQuickBase\(true, false\);[\s\S]*uploadPendingRecordsAuto\('poller'\);/.test(utilSource), 'Intake poller runs ingestion and Daily Upload only after a detected change');
...
assert(/intakePoller: \{[\s\S]*triggerCount: _listProjectTriggersByHandler_\('runIntakePoller'\)\.length[\s\S]*lastProcessedAt:/m.test(utilSource), 'Automation health reports intake poller status');
assert(/'intakeTriggers='/.test(utilSource), 'Automation health summary includes intake poller trigger count');
